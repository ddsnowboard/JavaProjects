...
fn best_strategies(s1: Strategy, s2: Strategy) -> (Strategy, Strategy) {
    fn best_strategy(other: Strategy) -> Strategy {
        let distances: Arc<Vec<_>> =
            Arc::new(VOTERS.par_iter().map(|v| distance(*v, other)).collect());
        *STRATEGIES
            .par_iter()
            .max_by_key(|s| {
                VOTERS
    ...
    (best_strategy(s2), best_strategy(s1))
}
...
fn simulate_iteration() {
    ...
    loop {
        println!("{:?}", (s1, s2));
        let new_strategies = best_strategies(s1, s2);
        if new_strategies == (s1, s2) {
            break;
        }
        let (new_s1, new_s2) = new_strategies;
        s1 = new_s1;
        s2 = new_s2;
    }
}
